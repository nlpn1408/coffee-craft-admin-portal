...
interface ProductVariantManagerProps { // Renamed props interface
    selectedProduct: Product | null;
    isViewMode?: boolean;
}

// Renamed component
const ProductVariantManager: React.FC<ProductVariantManagerProps> = ({ selectedProduct, isViewMode = false }) => {
    ...
    const columns = useVariantTableColumns({
        onEdit: (variant: ProductVariant) => { // Add explicit type here
    ...
    if (isVariantsError) {
         return <div className="p-4 text-center text-red-500">Failed to load variants.</div>;
    }
